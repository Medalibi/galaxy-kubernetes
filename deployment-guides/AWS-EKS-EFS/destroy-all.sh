...
if [ "$vpc_id" != 'None' ]; then
    ...
    security_group_id=$(aws ec2 describe-security-groups --region $AWS_REGION --filters "[{\"Name\": \"vpc-id\",\"Values\": [\"$vpc_id\"]}, {\"Name\": \"group-name\",\"Values\": [\"$SECURITY_GROUP_NAME\"]}]" --query "SecurityGroups[].GroupId" --output text)
